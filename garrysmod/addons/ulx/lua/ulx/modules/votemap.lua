...
function ulx.votemap( calling_ply, map )
	...
	-- TODO, color?
	ULib.tsay( _, string.format( "%s voted for %s (%i/%i). Say \"not votemap %i\" to vote for this map too.", calling_ply:Nick(), ulx.votemaps[ mapid ], mapvotes[ mapid ], votes_needed, mapid ), true )
	...
	if mapvotes[ mapid ] >= votes_needed then
		...
		if #admins <= 0 or vetotime < 1 then
			...
			for _, player in ipairs( admins ) do
				ULib.tsay( player, "To veto this vote, just say \"not veto\"", true ) -- TODO, color?
			end
