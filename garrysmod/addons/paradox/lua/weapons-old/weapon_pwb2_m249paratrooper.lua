if CLIENT then
    SWEP.WepSelectIcon = surface.GetTextureID("pwb2/vgui/weapons/m249paratrooper")
    SWEP.DrawWeaponInfoBox = false
    SWEP.BounceWeaponIcon = false
    killicon.Add("weapon_pwb2_m249paratrooper", "pwb2/vgui/weapons/m249paratrooper", Color(255, 255, 255, 255))
end

SWEP.PrintName = "M249 Paratrooper"
SWEP.Category = "PWB 2"
SWEP.Spawnable = true
...
SWEP.Primary.Sound = Sound("Weapon_PWB2_M249Paratrooper.Single")
...
function SWEP:Reload()
    if self.Reloading == 0 and self.ReloadingTimer <= CurTime() and self.Weapon:Clip1() < self.Primary.ClipSize and self.Weapon:Ammo1() > 0 then
        ...
        self.Iron = 0
        self.Reloading = 1
        self.ReloadingTimer = CurTime() + 4.5
        self.Sprint = 0
        self.Idle = 0
        self.IdleTimer = CurTime() + self.Owner:GetViewModel():SequenceDuration()
...
function SWEP:Think()
    ...
    if self.ReloadingTimer >= CurTime() + 4.25 then
    self.Owner:ViewPunch( Angle(0.015, -0.025, 0))
    end
