...
function SWEP:ShootEffects()
    ...
    self.Owner:SetEyeAngles(self.Owner:EyeAngles() + Angle(-1.5, math.Rand(-0.25, 0.25), 0))
end
...
function SWEP:IronIn()
    ...
    self.Owner:SetFOV(self.Owner:GetFOV() - 25, 0.6)
...
function SWEP:Think()
    ...
    if self.ReloadingTimer >= CurTime() + 5.75 then
    self.Owner:ViewPunch( Angle(0.015, 0, 0))
    end
    if self.ReloadingTimer >= CurTime() + 5.5 and self.ReloadingTimer < CurTime() + 5.75 then
    self.Owner:ViewPunch(Angle(0.025, 0.025, 0.015))
    end
    if self.ReloadingTimer >= CurTime() + 5.25 and self.ReloadingTimer < CurTime() + 5.5 then
    self.Owner:ViewPunch(Angle(0.025, 0.05, 0.025))
    end
    if self.ReloadingTimer >= CurTime() + 5 and self.ReloadingTimer < CurTime() + 5.25 then
    self.Owner:ViewPunch(Angle(0.015, 0.035, 0.05))
    end
    if self.ReloadingTimer >= CurTime() + 4.75 and self.ReloadingTimer < CurTime() + 5 then
    self.Owner:ViewPunch(Angle( -0.015, 0.015, 0.015))
    end
    if self.ReloadingTimer >= CurTime() + 4.5 and self.ReloadingTimer < CurTime() + 4.75 then
    self.Owner:ViewPunch(Angle(-0.025, 0, 0.015))
    end
    if self.ReloadingTimer >= CurTime() + 4.25 and self.ReloadingTimer < CurTime() + 4.5 then
    self.Owner:ViewPunch(Angle(0, 0, 0.015))
    end
    if self.ReloadingTimer >= CurTime() + 4 and self.ReloadingTimer < CurTime() + 4.25 then
    self.Owner:ViewPunch(Angle(0.025, 0.025, 0.015))
    end
    if self.ReloadingTimer >= CurTime() + 3.75 and self.ReloadingTimer < CurTime() + 4 then
    self.Owner:ViewPunch(Angle( 0.025, 0, 0.035))
    end
    if self.ReloadingTimer >= CurTime() + 3.5 and self.ReloadingTimer < CurTime() + 3.75 then
    self.Owner:ViewPunch(Angle(0, 0.025, 0.05))
    end
    if self.ReloadingTimer >= CurTime() + 3.25 and self.ReloadingTimer < CurTime() + 3.5 then
    self.Owner:ViewPunch(Angle(-0.025, 0, -0.035))
    end
    if self.ReloadingTimer >= CurTime() + 3 and self.ReloadingTimer < CurTime() + 3.25 then
    self.Owner:ViewPunch(Angle(-0.025, -0.015, -0.05))
    end
    if self.ReloadingTimer >= CurTime() + 2.75 and self.ReloadingTimer < CurTime() + 3 then
    self.Owner:ViewPunch(Angle(-0.05, -0.025, -0.025))
    end
