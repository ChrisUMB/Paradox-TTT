if CLIENT then
    SWEP.WepSelectIcon = surface.GetTextureID("pwb2/vgui/weapons/rpk")
    SWEP.DrawWeaponInfoBox = false
    SWEP.BounceWeaponIcon = false
    killicon.Add("weapon_pwb2_rpk", "pwb2/vgui/weapons/rpk", Color(255, 255, 255, 255))
end
...
SWEP.ViewModel = "models/pwb2/weapons/v_rpk.mdl"
...
SWEP.Primary.Sound = Sound("Weapon_PWB2_RPK.Single")
...
SWEP.Primary.SpreadKickIron = 0.006
...
function SWEP:ShootEffects()
    ...
    self.Owner:ViewPunch(Angle(-1.25, 0, math.Rand(-1, 1)))
    self.Owner:SetEyeAngles(self.Owner:EyeAngles() + Angle(-1.25, math.Rand(-0.25, 0.25), 0))
end
...
function SWEP:Reload()
    if self.Reloading == 0 and self.ReloadingTimer <= CurTime() and self.Weapon:Clip1() < self.Primary.ClipSize and self.Weapon:Ammo1() > 0 then
        ...
        self.Iron = 0
        self.Reloading = 1
        self.ReloadingTimer = CurTime() + 4.75
        self.Sprint = 0
        self.Idle = 0
        self.IdleTimer = CurTime() + self.Owner:GetViewModel():SequenceDuration()
...
function SWEP:Think()
    ...
    if self.ReloadingTimer >= CurTime() + 4.75 then
