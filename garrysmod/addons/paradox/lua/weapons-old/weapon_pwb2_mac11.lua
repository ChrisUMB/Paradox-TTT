if CLIENT then
    SWEP.WepSelectIcon = surface.GetTextureID("pwb2/vgui/weapons/mac11")
    SWEP.DrawWeaponInfoBox = false
    SWEP.BounceWeaponIcon = false
    killicon.Add("weapon_pwb2_mac11", "pwb2/vgui/weapons/mac11", Color(255, 255, 255, 255))
end
...
SWEP.ViewModel = "models/pwb2/weapons/v_mac11.mdl"
...
SWEP.Primary.Sound = Sound("Weapon_PWB2_MAC11.Single")
...
function SWEP:ShootEffects()
    ...
    self.Owner:SetEyeAngles(self.Owner:EyeAngles() + Angle(-2, math.Rand(-0.25, 0.25), 0))
end
...
function SWEP:Reload()
    if self.Reloading == 0 and self.ReloadingTimer <= CurTime() and self.Weapon:Clip1() < self.Primary.ClipSize and self.Weapon:Ammo1() > 0 then
        ...
        self.Iron = 0
        self.Reloading = 1
        self.ReloadingTimer = CurTime() + 2.5
        self.Sprint = 0
        self.Idle = 0
        self.IdleTimer = CurTime() + self.Owner:GetViewModel():SequenceDuration()
...
function SWEP:Think()
    ...
    if self.ReloadingTimer >= CurTime() + 2.25 then
    self.Owner:ViewPunch( Angle(-0.015, -0.025, -0.025 ))
    end
    ...
    self.Owner:ViewPunch(Angle( -0.035, 0.025, 0.025))
    end
