if CLIENT then
    SWEP.WepSelectIcon = surface.GetTextureID("pwb2/vgui/weapons/mp5a3")
    SWEP.DrawWeaponInfoBox = false
    SWEP.BounceWeaponIcon = false
    killicon.Add("weapon_pwb2_mp5a3", "pwb2/vgui/weapons/mp5a3", Color(255, 255, 255, 255))
end
...
SWEP.Primary.Sound = Sound("Weapon_PWB2_MP5A3.Single")
...
SWEP.Primary.SpreadKickIron = 0.0065
...
function SWEP:Think()
    ...
    if self.ReloadingTimer >= CurTime() + 2.75 then
    self.Owner:ViewPunch( Angle(-0.015, 0.025, 0.015))
    end
    if self.ReloadingTimer >= CurTime() + 2.5 and self.ReloadingTimer < CurTime() + 2.75 then
    self.Owner:ViewPunch(Angle( -0.035, 0.025, 0.05))
    end
    if self.ReloadingTimer >= CurTime() + 2.25 and self.ReloadingTimer < CurTime() + 2.5 then
    self.Owner:ViewPunch(Angle(-0.015, 0.015, 0.025))
    end
