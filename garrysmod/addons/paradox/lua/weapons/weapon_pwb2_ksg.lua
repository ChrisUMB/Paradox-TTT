--if CLIENT then
--    SWEP.WepSelectIcon = surface.GetTextureID("pwb2/vgui/weapons/ksg")
--    SWEP.DrawWeaponInfoBox = false
--    SWEP.BounceWeaponIcon = false
--    killicon.Add("weapon_pwb2_ksg", "pwb2/vgui/weapons/ksg", Color(255, 255, 255, 255))
--end
--
...
--SWEP.ViewModel = "models/pwb2/weapons/v_ksg.mdl"
...
--
--SWEP.Primary.Sound = Sound("Weapon_PWB2_KSG.Single")
...
function SWEP:PrimaryAttack()
    ...
    bullet.Dir = (tr.HitPos - self.Owner:GetShootPos()):GetNormalized() + (self.Owner:EyeAngles():Right() + self.Owner:EyeAngles():Up()) * (self.Primary.Spread - self.Primary.SpreadMin ) / (self.Primary.SpreadMax - self.Primary.SpreadMin) * math.Rand( -self.Primary.SpreadMax, self.Primary.SpreadMax)
...
function SWEP:ShootEffects()
    ...
    self.Owner:ViewPunch(Angle(-2, 0, math.Rand(-1, 1)))
    self.Owner:SetEyeAngles(self.Owner:EyeAngles() + Angle(-2, math.Rand(-0.5, 0.5), 0))
end
...
function SWEP:Think()
    ...
    if self.Iron == 1 and self.PumpTimer <= CurTime() and !self.Owner:KeyDown(IN_ATTACK2) then
    self.Weapon:IronOut()
    end
    ...
    self.Weapon:ReloadInsert()
    end
    if self.ReloadingTimer <= CurTime() and (self.Weapon:Clip1() == self.Primary.ClipSize || (self.Weapon:Clip1() > 0 and self.Weapon:Ammo1() <= 0)) then
